...
def generate_demo_videos():
    ...
    public_video_dir = "./bavis-frontend/public/videos"
    ...
    for filename, label, color in cameras:
        ...
        pub_filepath = os.path.join(public_video_dir, filename)
        ...
        if not os.path.exists(pub_filepath) and os.path.exists(filepath):
            shutil.copy(filepath, pub_filepath)
            print(f"Copied video to frontend public folder: {pub_filepath}")
